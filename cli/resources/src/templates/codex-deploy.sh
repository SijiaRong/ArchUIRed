...
# deploy.sh — Codex adapter deployment script (embedded in the CLI)
# Executed by `archui init` with cwd set to the target project root.
#
# Writes AGENTS.md with ArchUI agent instructions for Codex.
# Idempotent: re-running overwrites AGENTS.md with canonical content.
...
Reads `.archui/conversion-plan.yaml` and converts an existing project into ArchUI module structure. Run `node cli/resources/dist/index.js validate .` after completion.

### reconstruct-project

Reconstructs a project from its ArchUI spec layer. Reads `.archui-temp/` for original files and rebuilds `resources/` from spec.
